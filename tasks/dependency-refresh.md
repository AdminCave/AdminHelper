...
Status: erledigt · Branch: feature/dependency-refresh · Commit-Granularität: pro Task · Review: pro Task (feature-review) · Modell: Opus
...
Verify: `docker run --rm -v "$PWD:/w" -w /w python:3.12-slim sh -c "pip install -q pip-audit && pip-audit -r apps/server/requirements.txt --disable-pip && pip-audit -r apps/ca-issuer/requirements.txt --disable-pip"` → keine Funde. **Danach zwingend** die Code-Kompatibilität: `bash scripts/tests/run.sh quick --strict --only server ca-issuer` (der quick-Layer enthält ruff; seit `layer_lint` auf `apps/*/.venv/bin/ruff` zurückfällt, braucht es den expliziten Zusatzaufruf von damals nicht mehr). Bricht eine cryptography-API weg (CSR-Parsing/Signieren in `apps/ca-issuer/app/{pki,issuer,storage}.py`, `apps/server/app/core/identity.py`), ist das ein echter Code-Fix → nicht raten, `[?]` setzen.
...
Verify: je Projekt `npm audit --audit-level=high` ohne Fund, danach `bash scripts/tests/run.sh quick --strict --only web desktop-ui desktop-e2e` — svelte-check, eslint und die Unit-Suiten müssen grün bleiben.
Doku: CHANGELOG (Security) — im selben Commit
...
Verify: `cargo audit` im selben Verzeichnis meldet 0 Vulnerabilities (die unmaintained-GTK3-Warnungen dürfen bleiben), dann `bash scripts/dev/verify.sh desktop-rs --strict` — `cargo fmt --check`, `cargo clippy -- -D warnings`, `cargo test` grün.
Doku: CHANGELOG (Security) — im selben Commit
...
## Abschluss-Notiz (2026-09-07, ersetzt die Fassung vom 03.09.)
...
**mTLS-/PKI-Pfad real bestätigt** (Multibox `--agents 1 --enforce --desktop`,
11 ok / 1 failed): Agent über echten Netz-Hop provisioniert und mTLS-enrolled,
aus dem `:8445`-Repo installiert, CA-gepinnter Steady State, Monitoring hat
8 Reports verarbeitet. cryptography 50 trägt den Pfad.

**Das GUI-E2E-Gate für Variante (b) ist bestanden.** Der rote Lauf vom 03.09.
war ein Artefakt meines Versuchsaufbaus, nicht ein Befund: Ich hatte gegenüber
dem Vergleichslauf ZWEI Variablen gleichzeitig geändert (Branch-Stand *und*
Flags). Der saubere Vergleich am 07.09. mit identischen Flags:

| Lauf | Flags | Ergebnis |
|---|---|---|
| `main` | `--agents 0 --desktop` | DESKTOP_ALL_OK (2 specs), 8 ok / 0 failed |
| Branch | `--agents 0 --desktop` | DESKTOP_ALL_OK (2 specs), 8 ok / 0 failed |
| Branch | `--agents 1 --enforce --desktop` | beide Specs rot |

Beide Specs (`server-crud.live.js`, `monitoring-check.live.js`) laufen auf dem
Branch genauso grün wie auf `main`. Die Dependency-Updates — insbesondere die
erzwungenen `overrides` für `deepmerge-ts` und `@puppeteer/browsers` — brechen
die WebdriverIO-Harness NICHT. Variante (b) trägt.

**Eigenständiger Befund, NICHT Teil dieses Vorhabens:** Der Desktop-GUI-Lauf
scheitert unter `--enforce` (`MTLS_ENFORCE=true`) reproduzierbar an beiden
Specs, während er ohne das Flag auf demselben Stand grün ist. Das deutet
darauf hin, dass die Tauri-GUI bei erzwungenem mTLS nicht am Gateway
vorbeikommt. Das ist unabhängig von den Dependencies und gehört in ein eigenes
Vorhaben.

**Nebenbefund zur Testinfrastruktur:** Das Leasen scheitert sporadisch (heute
1 von 3 Erstversuchen). Ursache serverseitig bewiesen: alle `qmclone`-Tasks
laufen `OK` durch; crabbox' Proxmox-Provider bricht das Task-Polling beim
ersten abgerissenen Keep-Alive ab (`pveproxy` schließt jede Verbindung), ohne
den idempotenten GET zu wiederholen. Weder Zugangsdaten (Token ohne Ablauf,
alle Rechte) noch Host (7 TB frei) noch unsere Zeitgrenzen sind beteiligt.
Ein Retry um den Lease-Aufruf herum macht das beherrschbar — im Repo fehlt er
ausgerechnet an dieser teuersten Stelle.
