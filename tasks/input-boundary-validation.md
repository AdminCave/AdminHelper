...
Beim Bau (2026-09-22): Der Lauf hat geliefert, wofür er da war. Von 27 Server-Ausschlüssen waren zunächst 12 ganz weg und fünf auf check-granular geschrumpft; **T14 hat acht davon zurückgeholt** (siehe dort). Endstand, gegen `git show main:` nachgezählt: 27 vorher, 29 nachher — zwei entfernt, acht verengt, vier hinzugekommen (Monitoring-Proxy), acht als Voll-Ausschluss zurückgeholt (T14). Im Monitoring sind die drei `offset`-Einträge weg, der `tag-sync`-Eintrag bleibt. Dabei kam **ein neuer Produktfehler derselben Klasse** zum Vorschein, den vorher ein Ausschluss verdeckt hatte: ein NUL-Byte im `script` von `POST /api/hooks` (`DataError` beim INSERT, ungefangen). Er ist behoben, nicht ausgeschlossen — `SafeText` auf den Textfeldern von `HookCreate`/`HookUpdate`, plus Test. Zweiter Treffer war `GET /api/audit` mit `negative_data_rejection` (Pydantic-lax-Koerzierung plus die bekannte Header-Blindstelle) — bekanntes Framework-Verhalten, deshalb wieder ein Eintrag, aber check-granular und mit der Begründung aus genau diesem Lauf.
...
### T15 — Zwei Routen brauchen ihren Check-Ausschluss zurück  [x]
Komponente: server · Dateien: apps/server/tests/schemathesis_exclude.toml, CHANGELOG.md
Evidenz: run.sh[quick]: 4 passed, 0 failed, 13 skipped @5ef8d36f 2026-09-23T03:20:10+02:00
Review: approve (sonnet, 2 Runden, Lese-Prüfung + HTTP-Messung)
Änderung: Dieselbe Bauart wie T14, diesmal von der Suite selbst gefunden statt vom Review: der abschließende `run.sh quick` ist an `GET /api/notifications` mit `negative_data_rejection` gefallen. Beim Aufräumen habe ich den Eintrag ganz entfernt, weil seine dokumentierte Begründung (der `offset`-Überlauf) behoben ist — die Route hat aber denselben zweiten Grund wie ihre drei Geschwister `list_hooks`, `list_servers` und `get_connections`, die ich zu Recht check-granular behalten habe: FastAPI ignoriert unbekannte Query-Parameter, der Check wertet die 200 als akzeptierte schema-widrige Daten. Nachgestellt 2026-09-23: `GET /api/notifications?unknown_param=1` ⇒ 200. Beim Nachsehen fiel derselbe Fall bei `mark_read` auf, dessen alter Eintrag den zweiten Grund sogar ausdrücklich nannte: `{"ids": ["5"]}` ⇒ 200 und `{"ids": [true]}` ⇒ 200 (Pydantic koerziert im lax mode), `{"unknown": 1}` ⇒ 200. Beide bekommen einen Eintrag mit `checks = ["negative_data_rejection"]` — nicht die ganze Operation, damit `not_a_server_error` weiterläuft, und mit der Begründung aus diesem Lauf. Der Review zu dieser Task hat noch zwei weitere gefunden, die ich ebenfalls anhand ihrer alten Begründung entfernt hatte: `PUT /api/users/{user_id}` (`{"is_admin": 1}`, `{"is_admin": "yes"}`, `{"unknown_field": 1}` ⇒ je 200) und `POST /api/enrollment/token/for` (`{"browser": 1}`, `{"unknown_field": 1}` ⇒ je 200), beide gegen einen **existierenden** Benutzer nachgestellt — mit einer unbekannten Id antwortet die Route 404, und 404 steht auf der Erwartungsliste des Checks, der Treffer bliebe also verborgen. Von den ursprünglich 27 Ausschlüssen sind damit nur zwei wirklich verschwunden; alles andere ist entweder verengt oder mit neuer Begründung zurückgeholt. Das ist die ehrliche Bilanz dieses Vorhabens.
Verify: bash scripts/dev/verify.sh server --strict
...
### T17 — POST /api/api-keys: NUL im name  [x]
Komponente: server · Dateien: apps/server/app/modules/api_keys/schemas.py, apps/server/tests/test_text_bounds.py
Evidenz: run.sh[quick]: 4 passed, 0 failed, 13 skipped @7bdb37ce 2026-09-23T08:24:51+02:00
Review: approve (Fund des Verify-Laufs, Fix nach dem Muster von T10/T14)
Änderung: Vierter Fund derselben Klasse, diesmal vom Verify-Lauf zu T16 geliefert: `ApiKeyCreate.name` ist ein roher `str` und landet in `Column(String)` — `POST /api/api-keys` mit `{"name": "\u0000"}` stirbt beim INSERT mit `psycopg.DataError`, ungefangen, HTTP 500. Die Operation stand nie unter einem Ausschluss, der Fuzzer trifft sie nur, wenn Hypothesis für `name` ein NUL zieht; genau das erklärt vermutlich auch den einen unerklärten roten Lauf, den adminhelper-04 am 2026-09-23 um 06:42 hatte und dessen Ausgabe verloren ging. `name` bekommt `SafeText`, mit Test. `permission` ist ein `Literal` und braucht nichts.
Verify: bash scripts/dev/verify.sh server --strict
Doku: keine (die CHANGELOG-Liste der NUL-Stellen wird in T16 mitgezogen)
