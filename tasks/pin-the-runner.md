...
Re-Review (Opus, frischer Kontext): `approve`, kein neuer Defekt. Fünf Nits miterledigt: der Installer-Befehl in der Doku liest die Version aus der Datei statt sie zu wiederholen; der Hinweis bei fehlender CLI druckt den Befehl ohne Klammern und ist per Quelltext-Anker getestet; die Versionsschleife hat einen Positivfall (gleicher Baum, gleiche Locale, gültige Version ⇒ rc 0); `setup-token` läuft mit `env DISABLE_AUTOUPDATER=1`; der Kopf von `runner-redteam.sh` zählt die Proben richtig. Evidenz: runner_setup_test 68/0 · redteam_test 19/0 · hooks_test 172/0 · doc_smoke 15/0 · run.sh[quick]: 5 passed, 0 failed, 12 skipped (scripts).
