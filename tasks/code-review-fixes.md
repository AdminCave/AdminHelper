...
Verify: `bash scripts/dev/verify.sh monitoring --strict -- tests/` mit neuen Fällen: (a) `smart_health`-Check → 409, State unverändert; (b) deaktivierter `ping`-Check → 409; (c) aktiver `ping`-Check → weiterhin 200 und `execute_check` lief. Dann `bash scripts/dev/verify.sh monitoring --strict`.
Doku: CHANGELOG (Fixed) — im selben Commit

### T2 — Desktop-UI: keine tote Aktion mehr anbieten  [x] (beide Aufrufstellen gesperrt mit Begruendung im Tooltip, i18n DE+EN, Doku DE+EN. Push-Liste einmalig im UI-Modell und per neuem Sync-Guard an check_types.py gepinnt. Review fand zwei echte Fehler: 'ausgegraut' existierte im CSS gar nicht — der Knopf sah unveraendert aus und tat nichts, waehrend CHANGELOG und beide Doku-Dateien es behaupteten; und die UI-Kopie der Push-Liste hatte kein Gate. Beides behoben, Drift in beide Richtungen gegengeprueft.)
...
Verify: `bash scripts/dev/verify.sh desktop-ui --strict` (svelte-check, eslint, vitest) mit einem neuen Komponententest: bei `check_type='smart_health'` und bei `enabled=false` ist der Run-Button nicht klickbar, bei `ping` + enabled schon.
...
Verify: `bash scripts/dev/verify.sh server --strict -- tests/` mit einem neuen Test, der `subprocess.Popen` auf `OSError` patcht: nach N Aufrufen ist der Semaphore-Zähler unverändert und ein anschließender regulärer Hook läuft noch (auf dem alten Code rot). Dann `bash scripts/dev/verify.sh server --strict`.
Doku: CHANGELOG (Fixed) — im selben Commit
...
Verify: `bash scripts/dev/verify.sh server --strict -- tests/` mit einem Test: `X-Internal-Key` mit Nicht-ASCII-Zeichen ergibt 403 (nicht 500); gültiger Key weiterhin 202; leerer erwarteter Key lehnt weiterhin alles ab.
Doku: keine (intern)
