...
export const translations: Translations = {
  de: {
    ...
    "import.confirmReplace": "Alle bestehenden Verbindungen werden gelöscht und durch {count} importierte ersetzt. Fortfahren?",
    ...
    "confirm.server.delete": "Server wirklich löschen? Zugeordnete Verbindungen werden zu Standalone.",
    ...
    // ── Hook Script Help ──────────────────────────────────────────────
    "hook.scriptHelp.webhook": "Kontext: <code>payload</code>, <code>headers</code>, <code>params</code>",
    ...
    "hook.scriptHelp.base": "Verfügbar: <code>load_connections()</code>, <code>save_connections(list)</code>, <code>uuid4()</code>, <code>result</code>, <code>logs</code>, <code>log(msg)</code> – <code>import</code> erlaubt",
    ...
    "page.frp.noConfig": "Noch keine FRP-Server Konfiguration vorhanden. Klicke auf \"Konfigurieren\" um zu starten.",
    ...
    "frp.provision.hint": "Erstelle einen einmaligen Provision-Token (24h gültig). Der Token wird nur einmal angezeigt und kann nur einmal verwendet werden.",
    ...
    "modal.check.serviceHint": "Prueft systemd-Services. Im Auto-Modus werden failed und inaktive Autostart-Services automatisch erkannt.",
    ...
    "modal.check.proxmoxHint": "Prueft ob VMs/CTs aktuelle Backups haben. Plugin wird automatisch aktiviert wenn Proxmox erkannt wird.",
    ...
    "modal.check.zfsHint": "Prueft ZFS Pool Health und Kapazitaet. Plugin wird automatisch aktiviert wenn ZFS erkannt wird.",
    "modal.check.zfsCapWarn": "Kapazitaet Warnung (%)",
    "modal.check.zfsCapCrit": "Kapazitaet Kritisch (%)",
    "modal.check.dockerHint": "Prueft Docker Container Status. Plugin wird automatisch aktiviert wenn Docker erkannt wird.",
    ...
    "modal.template.placeholderHint": "Verwende <code>{{hostname}}</code>, <code>{{server_name}}</code>, <code>{{server_id}}</code> als Platzhalter",
    ...
    "passwordPrompt.hint.withRemember": "Bitte Benutzername und Passwort eingeben. Optional im System-Keyring speichern.",
  ...
  en: {
    ...
    "import.confirmReplace": "All existing connections will be deleted and replaced by {count} imported ones. Continue?",
    ...
    // ── Hook Script Help ──────────────────────────────────────────────
    "hook.scriptHelp.webhook": "Context: <code>payload</code>, <code>headers</code>, <code>params</code>",
    ...
    "hook.scriptHelp.base": "Available: <code>load_connections()</code>, <code>save_connections(list)</code>, <code>uuid4()</code>, <code>result</code>, <code>logs</code>, <code>log(msg)</code> – <code>import</code> allowed",
    ...
    "confirm.hook.rotateToken": "Really regenerate token for \"{name}\"? The old token will be invalidated.",
    ...
    // ── FRP Provisioning ──────────────────────────────────────────────
    "frp.provision.title": "Provisioning: {name}",
    "frp.provision.hint": "Create a one-time provision token (valid for 24h). The token is shown only once and can only be used once.",
    ...
    "modal.check.serviceHint": "Checks systemd services. In auto mode, failed and inactive autostart services are detected automatically.",
    ...
    "modal.check.proxmoxHint": "Checks whether VMs/CTs have current backups. Plugin is automatically activated when Proxmox is detected.",
    ...
    "modal.check.zfsHint": "Checks ZFS pool health and capacity. Plugin is automatically activated when ZFS is detected.",
    "modal.check.zfsCapWarn": "Capacity warning (%)",
    "modal.check.zfsCapCrit": "Capacity critical (%)",
    "modal.check.dockerHint": "Checks Docker container status. Plugin is automatically activated when Docker is detected.",
    ...
    "modal.template.placeholderHint": "Use <code>{{hostname}}</code>, <code>{{server_name}}</code>, <code>{{server_id}}</code> as placeholders",
    ...
    "passwordPrompt.hint.withRemember": "Please enter username and password. Optionally stored in system keyring.",
