...
1. **Schnell (überall, PR-CI):** Lint + Unit je Komponente (Tabelle in `CLAUDE.md`). Sammelbefehl
   `bash scripts/tests/run.sh [lint|unit|quick|integration|e2e|all]`, Default `quick`; dep-gated, Exit 75 = SKIP,
   am Ende `N passed, M failed, K skipped` (Exit ≠ 0 bei Fail).
...
3. **Multi-Host (Capstone):** `bash scripts/tests/crabbox_multibox.sh --agents N [--desktop]` — echtes `.deb` über
...
- **SKIP ≠ grün.** Exit 75 heißt „nicht verifiziert". Ein Live-Skript, das mangels Toolchain mit `exit 0` endet, ist
  ein Bug (Stufe 1 stellt alle auf 75 um). Ein erst roter, dann grüner Test ist `flaky`, kein PASS.
- **Nie grün melden ohne bestandene Suite.** Die Summary-Zeile ist die Evidenz und wird zitiert.
- **Verify-Zeilen in Ledgern** nur in Flag-Form: `bash scripts/tests/run.sh <layer> …`, ab Stufe 1 auch
  `bash scripts/dev/verify.sh <komponente> [--strict]`. Nie `FOO=bar cmd` — die Allow-Regel matcht nicht über die
  Zuweisung. Schwere Läufe stehen als eigene Zeile `Heavy:`. Env-Bedarf löst das Skript auf, nicht der Aufrufer.
- **Server-pytest** braucht `DATABASE_URL="$AH_TEST_DB"` aus `.devenv.sh` (bis `verify.sh` das in Stufe 1 übernimmt).
...
- **Plattform-Code** wird auf der Plattform verifiziert: Linux auf der VM; Windows bis Stufe 12 nur per
  `GOOS=windows go build` und CI-Job `rust-windows` — das ist dann ausdrücklich „nicht verifiziert".
