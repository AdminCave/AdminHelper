...
- **Tests must pass.** Before opening a PR, run the checks for the component you
  ...
  - Server / Monitoring (`server/`, `monitoring/`): `pytest -q`
  - Go agent (`agent-go/`): `gofmt -l .`, `go vet ./...`, `go test ./...`
  - Desktop UI / Web frontend (`desktop-src/`, `frontend-src/`): `npm run check`,
    `npm run lint`, `npm run test` (frontend additionally `npm run test:e2e`)
  - Desktop backend (`desktop/src-tauri/`): `cargo fmt --check`,
